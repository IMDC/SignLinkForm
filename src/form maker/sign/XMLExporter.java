 ...
 * 
 * @version $Id: XMLExporter.java 52 2007-11-08 03:56:17Z martin $
 */
public class XMLExporter
{
	...
	private ArrayList<Sign>	signLinks;
...
// IT'S ALIVE
	public XMLExporter(String sURL, String dURL, int dur, int tScale, ArrayList<Sign> sLinks, String html)
	{
		sourceFileURL = sURL;
		exportFileURL = dURL;
	...
	private String buildSigns(String path)
	{
		...
		for (int i = 0; i < signLinks.size(); i++)
		{
			...
			mStart = current.getMStart();
			mEnd = current.getMEnd();
			fTime = current.getFTime();
			fX = current.getX();
			fY = current.getY();
			fWidth = current.getWidth();
			fHeight = current.getHeight();
			url = current.getUrl();
			newWindow = current.isNewWindow();
