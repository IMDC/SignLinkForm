		...
		if (e != null && e.getSource().toString().contains("Main")) // if the slider from the main frame is calling the event
		{
			...
			if (frame.getSComp().getSelected() == -1) // if nothing is selected
			{
				...
				{ // activate buttons after the 3rd second
					...
					{
						if (vSlide.getSigns().get(i).getStartTime() < vSlide.getValue())
							minimum = i;
					}
					if (vSlide.getSigns().size() > minimum + 1)
						maximum = minimum + 1;
					if (minimum > -1)
						minTime = vSlide.getSigns().get(minimum).getEndTime() + ((int) (TimingComponent.MINSIGN));
					else
						minTime = -1;
					if (maximum > -1)
						maxTime = vSlide.getSigns().get(maximum).getStartTime() - (int) (TimingComponent.MINSIGN );
					else
						maxTime = -1;
					// select the active and enable buttons for manipulating it
					for (int i = 0; i < signs.size(); i++)
					{
						if (vSlide.getValue() >= signs.get(i).getStartTime() && vSlide.getValue() <= signs.get(i).getEndTime())
						{
			...
			{
				if (vSlide.getValue() > signs.get(frame.getSComp().getSelected()).getEndTime()
						|| vSlide.getValue() < signs.get(frame.getSComp().getSelected()).getStartTime())
				{
					...
					// if the preview button is clicked to go from the beginning of the sign to the end
					{
						...
						vSlide.setValue(signs.get(selected).getEndTime());
