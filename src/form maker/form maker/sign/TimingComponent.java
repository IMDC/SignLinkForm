...
public class TimingComponent extends JPanel
{
	...
	protected void initializeMinMax()
	{
		...
		else
		{
			...
			{
				if (slide.getSigns().get(i).getStartTime() < this.getStartTime())
					min = i;
			}
		...
		if (min > -1)
			setMinTime(slide.getSigns().get(min).getEndTime() + MINSIGN);
		else
			setMinTime(-1);
		if (max > -1)
			setMaxTime(slide.getSigns().get(max).getStartTime() - MINSIGN);
		else
			setMaxTime(-1);
	}
