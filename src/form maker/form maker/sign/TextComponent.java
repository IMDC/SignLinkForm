...
public class TextComponent extends JTextPane
{
	...
	public TextComponent(final String text)
	{
		...
		try
		{
			((HTMLDocument) this.getStyledDocument()).insertAfterStart(((HTMLDocument) this.getStyledDocument()).getElement(
					((HTMLDocument) this.getStyledDocument()).getDefaultRootElement(), StyleConstants.NameAttribute, HTML.Tag.BODY), "<pre>"+text+"</pre>");
		}
	...
	public String getExportText()
	{
		...
		if (this.getText().indexOf("<pre>")!=-1)
			return (this.getText().substring(this.getText().indexOf("<pre>") + 5, this.getText().lastIndexOf("</pre>") - 1));
		else
			return "";
	}
