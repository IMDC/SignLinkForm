 ...
 * @version $Id: ButtonsListener.java 52 2007-11-08 03:56:17Z martin $
 */

public class ButtonsListener implements ActionListener {
	private MenuFrame frame;
	private JFrame exportFrame;
	public static final String SIGNICONSLOCATION = "signlink_images"
			+ File.separator;
	...
	public void actionPerformed(ActionEvent event) {
		if (event.getActionCommand().equals("exit")) {
			try {
				if (event.getSource().getClass() == Class
						.forName("javax.swing.JMenuItem")) {
					frame = (MenuFrame) ((JMenu) ((JPopupMenu) ((JMenuItem) event
							.getSource()).getParent()).getInvoker())
							.getRootPane().getParent();
				} else
					// from the exit button or from the X button
					frame = (MenuFrame) ((JComponent) event.getSource())
							.getRootPane().getParent();
			...
			if (frame.isModified()) {
				int option = JOptionPane
						.showConfirmDialog(
								null,
								"You are about to exit SignLink Studio.\n The Project is not saved. Do you want to save the project?",
		...
		if (event.getActionCommand().equals("export")) {
			...
			exportFrame.setIconImage(Toolkit.getDefaultToolkit().getImage(
					this.getClass().getResource("/SignEd_icon_16.jpg")));
			exportFrame.setLocation((int) Toolkit.getDefaultToolkit()
					.getScreenSize().getWidth()
					/ 2 - width / 2, (int) Toolkit.getDefaultToolkit()
					.getScreenSize().getHeight()
					/ 2 - height / 2);
			exportFrame.getContentPane()
			...
			String exportPath = frame.getXmlFile().getParent() + File.separator
					+ xmlName.substring(0, xmlName.lastIndexOf("."));
			File exportFolder = new File(exportPath);
			if (!exportFolder.exists()) {
				exportFolder.mkdir();
			}
			final MovieExporter mExport = new MovieExporter(frame
					.getVComponent().getVideoPanel().getFile()
					.getAbsolutePath(), exportPath + File.separator
					+ xmlName.substring(0, xmlName.lastIndexOf(".") + 1)
					+ "flv", "flv");
			final SWFExporter swfExport = new SWFExporter(frame.getSComp()
					.getSignList(), frame.getXmlFile().getParent()
					+ File.separator + SIGNICONSLOCATION, exportPath, xmlName
					.substring(0, xmlName.lastIndexOf("."))
					+ ".flv", frame.getTPanel().getTextComponent()
					.getExportText());
		...
		if (event.getActionCommand().equals("Done")) // the done button in
		// the export frame is
		// clicked
		{
			exportFrame.dispose();
	...
	protected void saveAs() {
		...
		if (jfc.showSaveDialog(frame) == JFileChooser.APPROVE_OPTION) {
			...
			} else if (jfc.getSelectedFile().getName().toLowerCase().endsWith(
					".sgn")) {
				destinationFileURL = jfc.getSelectedFile().getAbsolutePath()
						.substring(
								0,
	...
	protected void save(String destinationFileURL) {
		...
		if (frame.getVComponent().getVideoPanel().getFile().getAbsolutePath()
				.endsWith(".flexTemp"+File.separator+"tmp.mp4")) {
			File destFile = new File(destinationFileURL.substring(0,
					destinationFileURL.lastIndexOf(".sgn"))
					+ ".mp4");
			try {
				SignUtils.copy(frame.getVComponent().getVideoPanel().getFile(),
						destFile);
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			new XMLExporter(destFile.getAbsolutePath(), destinationFileURL,
					frame.getVComponent().getVideoPanel().getDuration(), frame
							.getVComponent().getVideoPanel().getTimeScale(),
					frame.getSComp().getSignList(), frame.getTPanel()
							.getTextComponent().getExportText());
		} else {
			new XMLExporter(frame.getVComponent().getVideoPanel().getFile()
					.getAbsolutePath(), destinationFileURL, frame
					.getVComponent().getVideoPanel().getDuration(), frame
					.getVComponent().getVideoPanel().getTimeScale(), frame
					.getSComp().getSignList(), frame.getTPanel()
					.getTextComponent().getExportText());
		}
	}
}
