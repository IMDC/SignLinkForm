 ...
 * 
 * @version $Id: SignLink.java 52 2007-11-08 03:56:17Z martin $
 */
public class SignLink extends JFrame implements ActionListener, WindowListener
{
	...
	private final ImageIcon			underPreviewIcon		= new ImageIcon(Toolkit.getDefaultToolkit().getImage(
																	this.getClass().getResource("/images/redlink.gif")).getScaledInstance(
																	90, 20, 0));
	...
	private void setup(Sign signInfo, ArrayList<Sign> signs)
	{
		...
		imageTab.setGlassDimensions(signInfo.getX(), signInfo.getY(), signInfo.getWidth(), signInfo.getHeight());
		sign.setPreview(new ImageIcon(this.getImage(signInfo.getX(), signInfo.getY(), signInfo.getWidth(), signInfo.getHeight()).getImage()
				.getScaledInstance(90, 70, 0)));
	...
	private void addButtons()
	{
		...
		cancelButton = new JButton("Cancel", new ImageIcon(Toolkit.getDefaultToolkit()
				.getImage(this.getClass().getResource("/sCancel.jpg"))));
...
// }
	 ...
	 */
	/*
	 * protected void commitChanges() { imageTab.commitChanges(); addressTab.commitChanges(); timingTab.commitChanges();
...
// {
// this.getSign().setX(x);
// frame.getSign().setY(y);
// frame.getSign().setWidth(width);
// frame.getSign().setHeight(height);
// }
...
// SignLink.this.rollbackChanges();
		...
		{
			SignLink.this.getSign().setMStart(SignLink.this.timingTab.getTPanel().getTimingComponent().getStartTime());
			SignLink.this.getSign().setMEnd(SignLink.this.timingTab.getTPanel().getTimingComponent().getEndTime());
			SignLink.this.getSign().setNewWindow(SignLink.this.addressTab.getChecked());
			SignLink.this.getSign().setUrl(SignLink.this.addressTab.getLink());
			SignLink.this.getSign().setLabel(SignLink.this.addressTab.getLabel());
			SignLink.this.getSign().setFTime(SignLink.this.imageTab.getFrameTime());
			SignLink.this.getSign().setHeight(SignLink.this.imageTab.getGlassHeight());
			SignLink.this.getSign().setWidth(SignLink.this.imageTab.getGlassWidth());
			SignLink.this.getSign().setX(SignLink.this.imageTab.getGlassX());
			SignLink.this.getSign().setY(SignLink.this.imageTab.getGlassY());
			menuFrame.getVComponent().getVideoButtonsPanel().getVSlide().setValue(SignLink.this.getSign().getMStart());
